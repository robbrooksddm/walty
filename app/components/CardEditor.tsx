...
const handleProofAll = async () => {
  ...
  const sliderToZoom = (val: number) => {
    if (val <= 50) {
      const pct = 10 + (val / 50) * 89
      return pct / 100
    }
    const pct = 100 + ((val - 50) / 50) * 400
    return pct / 100
  }
  ...
  const setZoomSmooth = (val: number, origin: { x: number; y: number } | null) => {
  ...
  useEffect(() => {
    ...
    const wheel = (e: WheelEvent) => {
      if (e.ctrlKey || e.metaKey || e.altKey) {
        ...
        setZoomSmooth(
          targetZoom.current * Math.pow(0.999, delta),
    ...
    >
      ...
      <div className="fixed bottom-4 right-4 z-50 flex items-center gap-2 bg-white shadow px-3 py-2 rounded">
        ...
        <input
          ...
          max="100"
          step="1"
          value={zoomToSlider(targetZoom.current)}
          onChange={e =>
            setZoomSmooth(sliderToZoom(Number(e.currentTarget.value)), null)
          }
          className="h-2 w-32"
        />
      </div>
