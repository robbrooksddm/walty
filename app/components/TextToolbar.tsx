 ...
 * 2025-05-03  patch-live
 *   • keeps focus after every style change
 *   • mutations fire immediately (no setTimeout)
 *********************************************************************/
'use client'
...
export default function TextToolbar({
  ...
  if(!fc) return null            // guard (SSR mount)

  /* re-render when Fabric selection changes OR we mutate */
...
const mutate = (p: Partial<fabric.Textbox>) => {
  ...
  tb.set(p)                 // 1-a apply style
  tb.setCoords()

  fc.setActiveObject(tb)    // 1-b RE-SELECT immediately
  fc.requestRenderAll()

  // 2 fire modified *after* selection is locked in
  tb.fire('modified')
  fc.fire('object:modified', { target: tb })

  force({})                 // refresh toolbar UI
}
        ...
        <div className="toolbar pointer-events-auto flex flex-wrap items-center gap-2 border bg-white/95 backdrop-blur rounded-md shadow px-3 py-1 max-w-[800px] w-[calc(100%-10rem)]">
          ...
          <label className="relative cursor-pointer">
            ...
            <input type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer"
                   onChange={e=>{const f=e.currentTarget.files?.[0];if(f)addImage(f);e.currentTarget.value='';}}/>
          </label>

          {/* font family */}
          <select disabled={!tb} value={tb?tb.fontFamily??fonts[0]:''}
          ...
          <div className="flex items-center">
            ...
            <input disabled={!tb} type="number" value={tb?tb.fontSize:''}
          ...
          {/* align */}
          <select disabled={!tb} value={tb?tb.textAlign:''}
