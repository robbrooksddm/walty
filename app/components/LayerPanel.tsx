...
function Row({ id, idx }: { id: string; idx: number }) {
  ...
  const dropAfter =
    showGuide && newIndex === index + 1 && newIndex < total;
  ...
  return (
    ...
    >
      {dropBefore && (
        <div className="pointer-events-none absolute inset-x-0 -top-7 h-1 bg-walty-orange" />
      )}
      {dropAfter && (
        <div className="pointer-events-none absolute inset-x-0 -bottom-7 h-1 bg-walty-orange" />
      )}
      {/* drag handle */}
      <button
        {...listeners}
