...
const scrollAncestors = (el: HTMLElement | null) => {
...
const addScrollListeners = (
  ...
  const nodes = scrollAncestors(el)
  nodes.forEach(n => n.addEventListener('scroll', handler, { passive: true, capture: true }))
  window.addEventListener('resize', handler)
  return () => {
    nodes.forEach(n => n.removeEventListener('scroll', handler))
...
if (container) {
  ...
  updateOffset();
  const removeOffsetScroll = addScrollListeners(containerRef.current, updateOffset);
...
let removeHoverScroll: (() => void) | null = null
...
fc.on('selection:created', () => {
  ...
  removeScrollSync = addScrollListeners(containerRef.current, scrollHandler)
})
...
fc.on('mouse:over', e => {
  ...
  hoverDomRef.current && (() => {
    ...
    removeHoverScroll = addScrollListeners(containerRef.current, hoverScrollHandler)
  })()
})
.on('mouse:out', () => {
  hoverHL.visible = false
  hoverDomRef.current && (() => {
    ...
    if (hoverScrollHandler) {
      removeHoverScroll && removeHoverScroll()
...
doSync = () =>
  canvasRef.current && ghost && syncGhost(img, ghost, canvasRef.current)
            ...
            img.on('moving',   doSync)
               .on('scaling',  doSync)
               .on('rotating', doSync)
               const removeGhostScroll = addScrollListeners(containerRef.current, doSync)
