...
useEffect(() => {
  ...
  const handles = [...corners, 'rot'] as const;
  ...
  const bridge = (e: PointerEvent) => {
    ...
    const dx = corner && corner !== 'rot' && corner.includes('l') ? offset
             : corner && corner !== 'rot' && corner.includes('r') ? -offset
             : 0
    const dy = corner && corner !== 'rot' && corner.includes('t') ? offset
             : corner && corner !== 'rot' && corner.includes('b') ? -offset
             : 0
...
const drawOverlay = (
  ...
  if (el._handles) {
    ...
    if (h.rot) {
      const rotY = Math.round(height + ROT_HANDLE_OFFSET)
