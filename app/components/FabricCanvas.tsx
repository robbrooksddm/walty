...
useEffect(() => {
  ...
  const bridge = (e: PointerEvent) => {
    const datasetCorner = (e.target as HTMLElement | null)?.dataset.corner
    const corner = datasetCorner === 'rot' ? 'mtr' : datasetCorner
    const vt = fc.viewportTransform || [1, 0, 0, 1, 0, 0]
    const scale = vt[0]
    const offset = PAD * scale
    const dx =
      datasetCorner && datasetCorner !== 'rot' && datasetCorner.includes('l')
        ? offset
        : datasetCorner && datasetCorner !== 'rot' && datasetCorner.includes('r')
          ? -offset
          : 0
    const dy =
      datasetCorner && datasetCorner !== 'rot' && datasetCorner.includes('t')
        ? offset
        : datasetCorner && datasetCorner !== 'rot' && datasetCorner.includes('b')
          ? -offset
          : 0
...
const drawOverlay = (
  ...
  if (el._handles) {
    ...
    const objScaleY = obj.getObjectScaling().scaleY
    const rotY   = Math.round(height / 2 + ROT_HANDLE_OFFSET * objScaleY * scale)
