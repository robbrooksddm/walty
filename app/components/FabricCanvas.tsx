...
const drawOverlay = (
  ...
  const scaleX = vt[0]
  const scaleY = vt[3]
  const left   = rect.left + vt[4] + (box.left - PAD) * scaleX
  const top    = rect.top  + vt[5] + (box.top - PAD) * scaleY
  const width  = (box.width  + PAD * 2) * scaleX
  const height = (box.height + PAD * 2) * scaleY
...
fc.on('mouse:over', e => {
  ...
  const scaleY = vt[3]
  hoverDomRef.current && (() => {
    hoverDomRef.current.style.left = `${rect.left + vt[4] + (box.left - PAD) * scaleX}px`
    hoverDomRef.current.style.top = `${rect.top + vt[5] + (box.top - PAD) * scaleY}px`
    hoverDomRef.current.style.width = `${(box.width + PAD * 2) * scaleX}px`
    hoverDomRef.current.style.height = `${(box.height + PAD * 2) * scaleY}px`
