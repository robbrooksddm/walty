...
const drawOverlay = (
  ...
  if (el._handles) {
    ...
    const isCrop = el.classList.contains('crop-window')
    const leftX  = Math.round(isCrop ? 0 : half)
    const rightX = Math.round(isCrop ? width : width - half)
    const topY   = Math.round(isCrop ? 0 : half)
    const botY   = Math.round(isCrop ? height : height - half)
