...
if (container) {
  ...
  document.addEventListener('scroll', updateOffset, { passive: true, capture: true });
...
fc.on('mouse:over', e => {
  ...
  hoverDomRef.current && (() => {
    ...
    document.addEventListener('scroll', hoverScrollHandler, { passive: true, capture: true })
...
window.addEventListener('keydown', onKey)
  ...
  fcRef.current = fc; onReady(fc)
    ...
    return () => {
      ...
      document.removeEventListener('scroll', updateOffset)
...
doSync = () =>
  canvasRef.current && ghost && syncGhost(img, ghost, canvasRef.current)
            ...
            img.on('moving',   doSync)
               ...
               document.addEventListener('scroll', doSync, { passive: true, capture: true })
            ...
            img.on('removed', () => {
              ...
              document.removeEventListener('scroll', doSync)
