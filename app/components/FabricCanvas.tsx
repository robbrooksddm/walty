...
const drawOverlay = (
  ...
  if (el._handles) {
    ...
    const cx  = obj.left + boxW / 2
    const cy  = obj.top  + boxH
    const baseX = window.scrollX + scrollX + rect.left + vt[4]
    const baseY = window.scrollY + scrollY + rect.top  + vt[5]
    const rotX = baseX + (cx + ROT_HANDLE_OFFSET * sin) * scale
    const rotY = baseY + (cy + ROT_HANDLE_OFFSET * cos) * scale
    ...
    if (h.mtr) {
      h.mtr.style.left = `${Math.round(rotX - left)}px`
      h.mtr.style.top  = `${Math.round(rotY - top)}px`
    }
  }
}
