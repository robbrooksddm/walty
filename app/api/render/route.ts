...
export async function POST (req: NextRequest) {
  try {
    ...
    /* download GLB to bypass cross-origin restrictions */
    const modelRes    = await fetch(variant.model)
    const modelBuffer = Buffer.from(await modelRes.arrayBuffer())
    const modelData   = `data:model/gltf-binary;base64,${modelBuffer.toString('base64')}`
    ...
    const html = `<!DOCTYPE html>
      <html>
        ...
        <script type="module">
          ...
          const gltf = await gltfLoader.loadAsync('${modelData}');
