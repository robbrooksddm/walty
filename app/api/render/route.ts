...
export async function POST (req: NextRequest) {
  try {
    ...
    const threeBase = path.join(path.dirname(require.resolve('three')), '..')
    const threePath = path.join(threeBase, 'build/three.min.js')
    const gltfPath  = path.join(threeBase, 'examples/jsm/loaders/GLTFLoader.js')

    await page.addScriptTag({ path: threePath })
    await page.addScriptTag({ path: gltfPath })
