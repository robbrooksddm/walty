    ...
    const glContext = gl(width, height, { preserveDrawingBuffer: true }) as any
        ...
        const downgrade = (src: string) => {
          ...
          return trimmed
            ...
            // WebGL1 does not support integer or 3D samplers; fall back to 2D
            .replace(/[iu]?sampler3D/g, 'sampler2D')
            .replace(/[iu]?sampler2DArray/g, 'sampler2D')
            .replace(/[iu]?sampler2D/g, 'sampler2D')
            .replace(/[iu]?samplerCube/g, 'samplerCube')
            .replace(/[iu]?sampler2DShadow/g, 'sampler2D')
