...
export async function POST (req: NextRequest) {
  try {
    ...
    const threeSrc = await readFile(threePath, 'utf8')
    const gltfSrc  = await readFile(gltfPath, 'utf8')

    const threeUrl = `data:text/javascript;base64,${Buffer.from(threeSrc).toString('base64')}`
    const gltfUrl  = `data:text/javascript;base64,${Buffer.from(gltfSrc).toString('base64')}`
