  ...
  try {
    ...
    const glContext = gl(width, height, { preserveDrawingBuffer: true }) as any
        ...
        const downgrade = (src: string) => {
          if (!src.startsWith('#version 300 es')) return src
          return src
    ...
    // Force GLSL1 for all materials so Three.js generates WebGL1 shaders
    gltf.scene.traverse(obj => {
      const mat = (obj as any).material
      if (mat) (mat as any).glslVersion = THREE.GLSL1
    })
